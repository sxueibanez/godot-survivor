...
func update_progress():
	...
	var cost := upgrade.get_experience_cost(current_quantity)
	var is_maxed: bool = upgrade.max_quantity > 0 && current_quantity >= upgrade.max_quantity
	var currency = MetaProgression.save_data["meta_upgrade_currency"]
	var percent = currency / float(cost)
	...
	progress_label.text = "%d/%d" % [currency, cost]
	count_label.text = "×%d" % current_quantity
...
func on_purchase_pressed():
	if upgrade == null:
		return
	var cost := upgrade.get_experience_cost(MetaProgression.get_upgrade_count(upgrade.id))
