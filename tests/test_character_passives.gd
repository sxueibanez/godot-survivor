...
func _init() -> void:
	...
	assert(is_equal_approx(float(warrior.call("get_weapon_damage_multiplier", sword)), 1.2))
	...
	assert(is_equal_approx(float(elf_ranger.call("get_weapon_damage_multiplier", axe)), 1.2))
	...
	assert(is_equal_approx(float(blooddrinker.get("missing_health_damage_bonus_per_10")), 0.015))
	assert(is_equal_approx(float(blooddrinker.get("missing_health_speed_bonus_per_10")), 1.5))
